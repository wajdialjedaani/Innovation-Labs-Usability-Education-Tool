  ...
  return (
    <div>
      ...
      <p className={styles.sideBySide}>
        <img src="/lessons/lesson1-graphics/visibility1.gif" alt="Clicking a button notifies the user of the state of the program" />
        <div>
          The most important element of the system status that needs to be communicated to the user is <strong>appropriate
          feedback</strong> regarding their actions. For example, if a user clicks on an icon, something should visually
          indicate that an action was performed.
        </div>
      </p>
      <p>If not, the user will likely keep clicking the icon, leading to
        potentially unwanted behavior. Even if the result of an action is still being computed, or can not be
        communicated to the user, the action itself must be acknowledged as soon as possible. <strong>Immediate feedback </strong> 
      ...
      <p>Another important aspect of information sharing is <strong>trust</strong>. Sharing the system status will build trust with the
      ...
      <p>System status can be divided into two sections: <strong>persistent and situational</strong>. A situational system status depends on
      ...
      <p className={styles.sideBySide}>
        ...
        <div>
        A <strong>persistent</strong> system status is displayed constantly and does not depend on user input beforehand. An example is a
      ...
      <p>There are many elements of a system’s state that are of interest to the user, but there are many more
        ...
        <strong>Determining what status items are important involves extensive testing and feedback</strong>.
      </p>
    </div>
  )
}